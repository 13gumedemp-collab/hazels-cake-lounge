// Generates a branded PDF invoice for an order, stores it in the invoices
// bucket, emails it (invoice template), and sets invoice_sent = true.
...
export async function generateInvoice(supabase: SupabaseClient, order_id: string): Promise<{ status: string; error?: string }> {
  ...
  let y = height - 70;
  page.drawText("Hazel's Cake Lounge", { x: 50, y, size: 22, font: bold, color: INK }); y -= 22;
  ...
  const row = (label: string, value: string) => {
    ...
    page.drawText(value || "-", { x: 200, y, size: 11, font: serif, color: INK }); y -= 22;
  };
  ...
  row("Occasion", cm ? `${cm.person_name}'s ${cm.occasion_type}` : "-");
  ...
  y -= 16;
  page.drawText("Once your deposit is received I will confirm your order and get started.", { x: 50, y, size: 11, font: serif, color: GOLD });
  y -= 40;
  page.drawText("Thank you for trusting me with your celebration.", { x: 50, y, size: 12, font: serif, color: INK });
  y -= 18;
